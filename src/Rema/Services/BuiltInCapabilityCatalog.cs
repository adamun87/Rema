...
public static class BuiltInCapabilityCatalog
{
    ...
    [
        ...
        new("Tool", "ado_get_build_status",
            ...
            ["ado", "polling"]),
        new("Tool", "ado_trigger_pipeline",
            "Trigger a new pipeline run on a specific branch with branch verification.",
            "Queues an ADO pipeline build on an explicit source branch, confirms the branch matches, and auto-tracks the build in the active shift.",
            ["ado", "builds", "trigger"]),
        new("Tool", "ado_get_build_by_branch",
            "Find recent builds for a specific branch to verify correct artifacts before deploying.",
            "Filters recent pipeline builds by exact source branch. Use to confirm the right build artifact exists before triggering a deployment.",
            ["ado", "builds", "branch"]),
